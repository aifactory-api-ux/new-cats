...
- Name: buttons.spec.ts >> Botones - MiCuenta >> botón submit de formulario existe
- Location: tests-functional/buttons.spec.ts:88:3
...
Locator: getByRole('button', { name: /entrar|registrar|guardar/i }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /entrar|registrar|guardar/i }).first()
